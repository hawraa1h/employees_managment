...
@section('content')
    <main class="app-content">
        <div class="app-title">
            <h1>{{ __('إدارة السياسات') }}</h1>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="tile">
                    <div class="d-flex mainAdd">
                        <h3 class="tile-title">{{ __('قائمة السياسات') }}</h3>
                        @if(!hasRole('normal'))
                        <a href="{{ route('main_admin.policies.create') }}" class="btn float-right btn-primary"><i class="fa fa-plus"></i> إضافة</a>
                        @endif
                    </div>
...
{{--                                <td>{{ $policy->department->name }}</td>--}}
                                ...
                                <td>
                                    ...
                                    @if(!hasRole('normal'))
                                    <a href="{{ route('main_admin.policies.edit', $policy->id) }}" class="btn btn-info btn-sm">
                                    ...
                                    <form id="trash{{$policy->id}}" method="post" action="{{route('main_admin.policies.destroy', $policy->id)}}">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                    @endif
                                    ...
                                    @if ($policy->status == 'pending')
                                        @if(hasPermission('review_policies') && !hasRole('normal'))
                                        ...
                                        <div class="modal fade" id="reviewModal{{$policy->id}}" tabindex="-1" role="dialog" aria-labelledby="reviewModalLabel{{$policy->id}}" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form method="POST" action="{{ route('main_admin.policies.review', $policy->id) }}" enctype="multipart/form-data">
                                                        @csrf
...
{{--                                                            <p><strong>{{ __('القسم') }}:</strong> {{ $policy->department->name }}</p>--}}
                                    ...
                                    @if ($policy->status == 'reviewed')
                                        @if(hasPermission('audit_policies')&& !hasRole('normal'))
                                        ...
                                        <div class="modal fade" id="auditModal{{$policy->id}}" tabindex="-1" role="dialog" aria-labelledby="auditModalLabel{{$policy->id}}" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form method="POST" action="{{ route('main_admin.policies.audit', $policy->id) }}" enctype="multipart/form-data">
                                                        @csrf
                                                        <div class="modal-body">
                                                            <p><strong>{{ __('العنوان') }}:</strong> {{ $policy->title }}</p>
{{--                                                            <p><strong>{{ __('القسم') }}:</strong> {{ $policy->department->name }}</p>--}}
